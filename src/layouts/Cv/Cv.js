...
export default function Cv() {
	...
	const handleChange = (e) => {
		...
		// Handle multiple selection for cvEmployeeSkills
		if (name === 'cvEmployeeSkills[]') {
			const selectedSkills = Array.from(e.target.selectedOptions, (option) => option.value);
			setData((prevData) => ({ ...prevData, [name]: selectedSkills }));
		} else {
	...
	// For Getting Skills integration
	...
	return (
		<DashboardLayout>
			...
			<Grid className="cv_head">
				...
				<AddIcon className="cv_add" fontSize="medium" variant="outlined" onClick={handleClickOpen} />
			</Grid>
					...
					<Grid container spacing={4}>
						...
						<Grid item xs={6}>
							...
							<Select
								...
								multiple
								fullWidth
								defaultValue={Array.isArray(data.cvEmployeeSkills) ? data.cvEmployeeSkills : []}
								onChange={handleChange}
			...
			<Grid className="table_container">
				<TextField className="search_box" placeholder="Search..." onChange={(e) => setQuarry(e.target.value)} />
				<table>
					<tr>
						<th>Serial No.</th>
						<th>Employee Name</th>
